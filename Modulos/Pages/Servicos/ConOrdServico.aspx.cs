...
{
    public partial class ConOrdServico : System.Web.UI.Page
    {
        ...
        protected void MontaFiltro(object sender, EventArgs e)
        {
            ...
            Lista = d.ListarCamposView("VW_DOC_OS");
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Session["TabFocadaOrdemServico"] != null)
            {
                PanelSelect = Session["TabFocadaOrdemServico"].ToString();
            }
            else
            {
                PanelSelect = "home";
                Session["TabFocadaOrdemServico"] = "home";
                txtFiltro11.Focus();
            }

            if (Session["LST_ORDSERVICO"] != null)
            {
                listaT = (List<DBTabelaCampos>)Session["LST_ORDSERVICO"];
            ...
            {
                if (Session["ZoomOrdemServico2"].ToString() == "RELACIONAL")
                {
                    pnlPainel.Visible = false;
                    cmdSair.Visible = false;
                }
        ...
        protected void btnConsultar_Click(object sender, EventArgs e)
        {
            ...
            {
                strTipo = d.ListarTipoCampoView("VW_DOC_OS", lblFiltroData1.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoView("VW_DOC_OS", lblFiltroData2.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoView("VW_DOC_OS", lblFiltroData3.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoView("VW_DOC_OS", lblFiltro1.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoView("VW_DOC_OS", lblFiltro2.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoView("VW_DOC_OS", lblFiltro3.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoView("VW_DOC_OS", lblFiltro4.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoView("VW_DOC_OS", lblFiltro5.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoView("VW_DOC_OS", lblFiltro6.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoView("VW_DOC_OS", lblFiltro7.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoView("VW_DOC_OS", lblFiltro8.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoView("VW_DOC_OS", lblFiltro9.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoView("VW_DOC_OS", lblFiltro10.ToolTip).ToString().ToUpper();
            ...
            if (ddlClassificacao.SelectedValue == "1")
            {
                CodClassificacao = 97;
            }
            else if (ddlClassificacao.SelectedValue == "2")
            {
                CodClassificacao = 98;
            }

            listaT.RemoveAll(x => x.Filtro == "CD_CLASSIFICACAO");
            if (CodClassificacao != 0)
            {
                
                DBTabelaCampos rowp2 = new DBTabelaCampos();
                rowp2.Filtro = "CD_CLASSIFICACAO";
                rowp2.Inicio = CodClassificacao.ToString();
                rowp2.Fim = CodClassificacao.ToString();
                rowp2.Tipo = "INT";
                listaT.Add(rowp2);
            }

            Session["LST_ORDSERVICO"] = listaT;
            grdGrid.DataSource = r.ListarOrdemServicoCompleto(listaT);
            grdGrid.DataBind();
            if (grdGrid.Rows.Count == 0)
            {
                ShowMessage("Ordem(ns) de Serviço não encontrado(s) mediante a pesquisa realizada.", MessageType.Info);
        ...
        protected void grdGrid_SelectedIndexChanged(object sender, GridViewCommandEventArgs e)
        {
            ...
            Session["ZoomOrdemServico"] = Codigo;
            ...
            else if (x == "Imprimir")
            {
                ...
                doc = docDAL.PesquisarDocumento(Convert.ToDecimal(Codigo));
                GerarJPGporHTML(doc.DescricaoDocumento,doc.CodigoDocumento,0);

                if (doc.CodigoClassificacao == 98)
                {
                    List<Doc_OrdemServico> ListaOSs = new List<Doc_OrdemServico>();
                    ListaOSs = docDAL.ListarOrdemServicoServico(doc.Cpl_CodigoPessoa, 97, doc.CodigoDocumento);
                    foreach (Doc_OrdemServico os in ListaOSs)
                    {
                        ...
                        ListaItem = itemDAL.ObterItemDocumento(os.CodigoDocumento);
                        foreach (ItemDocumento item in ListaItem)
                        {
                            GerarJPGporHTML(item.DescricaoItem, os.CodigoDocumento, item.CodigoItem);
                        }
                ...
                else
                {
                    List<ItemDocumento> ListaItem = new List<ItemDocumento>();
                    ItemDocumentoDAL itemDAL = new ItemDocumentoDAL();
                    ListaItem = itemDAL.ObterItemDocumento(doc.CodigoDocumento);
                    foreach (ItemDocumento item in ListaItem)
                    {
                        GerarJPGporHTML(item.DescricaoItem, doc.CodigoDocumento, item.CodigoItem);
                    }
                }
        ...
        protected void GerarJPGporHTML(string CodigoFonteHTML, decimal CodigoDocumento, int CodigoItem)
        {
            string CaminhoArquivoLog = "";
            if(CodigoItem == 0)
                CaminhoArquivoLog = Path.GetDirectoryName(System.AppDomain.CurrentDomain.BaseDirectory.ToString()) + "\\Log\\ImagemRelatorio\\RelOrdServico" + CodigoDocumento + ".jpg";
            else
                CaminhoArquivoLog = Path.GetDirectoryName(System.AppDomain.CurrentDomain.BaseDirectory.ToString()) + "\\Log\\ImagemRelatorio\\RelOrdServico" + CodigoDocumento + "_"+CodigoItem+".jpg";

            System.Drawing.Image m_Bitmap = new Bitmap(750,750);
            PointF point = new PointF(0, 0);
            SizeF maxSize = new System.Drawing.SizeF(500, 500);

            TheArtOfDev.HtmlRenderer.WinForms.HtmlRender.RenderToImage(m_Bitmap, "<html><body style='background-color:white;margin:0;padding:0;width:750px;height:750px;'>" + CodigoFonteHTML + "</body></html>");
