...
{
    public partial class ConTipoOperacao : System.Web.UI.Page
    {
        ...
        protected void MontaFiltro(object sender, EventArgs e)
        {
            ...
            Lista = d.ListarCamposSQL("TIPO_DE_OPERACAO");
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Session["LST_TIPOOPERACAO"] != null)
            {
                listaT = (List<DBTabelaCampos>)Session["LST_TIPOOPERACAO"];
                if (listaT.Count != 0)
            ...
            if (Session["Pagina"].ToString() != Request.CurrentExecutionFilePath)
            {
                ...
                if (Session["MovimentacaoInterna"] != null && Convert.ToInt32(Request.QueryString["cad"]) == 1 || Session["ConMovEstoque"] != null && Convert.ToInt32(Request.QueryString["cad"]) == 2)
                {
                    btnNovo.Visible = false;
        ...
        protected void btnConsultar_Click(object sender, EventArgs e)
        {
            ...
            {
                strTipo = d.ListarTipoCampoSQL("TIPO_DE_OPERACAO", lblFiltro1.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoSQL("TIPO_DE_OPERACAO", lblFiltro2.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoSQL("TIPO_DE_OPERACAO", lblFiltro3.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoSQL("TIPO_DE_OPERACAO", lblFiltro4.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoSQL("TIPO_DE_OPERACAO", lblFiltro5.ToolTip).ToString().ToUpper();
            ...
            {
                strTipo = d.ListarTipoCampoSQL("TIPO_DE_OPERACAO", lblFiltro6.ToolTip).ToString().ToUpper();
            ...
            Session["LST_TIPOOPERACAO"] = listaT;
            TipoOperacaoDAL r = new TipoOperacaoDAL();
            grdGrid.DataSource = r.ListarTipoOperacoesCompleto(listaT);
            grdGrid.DataBind();
            if (grdGrid.Rows.Count == 0)
            {
                ShowMessage("Tipo(s) de Operação não encontrado(s) mediante a pesquisa realizada.", MessageType.Info);
                Session["LST_TIPOOPERACAO"] = null;
            }
        }
        protected void grdGrid_SelectedIndexChanged(object sender, EventArgs e)
        {
            Session["ZoomTipoOperacao"] = HttpUtility.HtmlDecode(grdGrid.SelectedRow.Cells[0].Text) + "³";

            if (Session["MovimentacaoInterna"] != null && Convert.ToInt32(Request.QueryString["cad"]) == 1)
            {
                Response.Redirect("~/Pages/Estoque/ManMovInterna.aspx");
            }
            if (Session["ConMovEstoque"] != null && Convert.ToInt32(Request.QueryString["cad"]) == 2)
            {
                Response.Redirect("~/Pages/Estoque/ConMovEstoque.aspx");
            }
            else
            {
                Response.Redirect("~/Pages/Fiscal/CadTipoOperacao.aspx");
            }
