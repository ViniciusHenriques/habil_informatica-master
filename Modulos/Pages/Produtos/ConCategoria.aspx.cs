...
{
    public partial class ConCategoria : System.Web.UI.Page
    {
        ...
        protected void MontaSelecionar(object sender, EventArgs e)
        {
            ...
            ddlPesquisa.DataSource = d.ListarCamposSQL("CATEGORIA");
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if ((Session["Pagina"].ToString() != Request.CurrentExecutionFilePath) || (ddlPesquisa.Text == ""))
            {
                Session["Pagina"] = Request.CurrentExecutionFilePath;
                    ...
                    grdGrid.DataSource = r.ListarCategorias("", "", "", "");
                    grdGrid.DataBind();
                    if (grdGrid.Rows.Count == 0)
                        ShowMessage("Não existem Categorias Cadastrados.",MessageType.Info);
                ...
                else if (Session["CadProduto"] != null)
                {
                    //btnImprimir.Visible = false;
                    btnSair.Visible = false;
                    btnVoltar.Visible = true;
        ...
        {
            DBTabelaDAL d = new DBTabelaDAL();
            String strTipo = d.ListarTipoCampoSQL("CATEGORIA", ddlPesquisa.SelectedValue).ToString().ToUpper();
            ...
            CategoriaDAL r = new CategoriaDAL();
            grdGrid.DataSource = r.ListarCategorias(ddlPesquisa.Text, strTipo, txtVarchar.Text , ddlPesquisa.Text);
            grdGrid.DataBind();
            if (grdGrid.Rows.Count == 0)
                ShowMessage("Categoria(s) não encontrado(s) mediante a pesquisa realizada.", MessageType.Info);
        }
        ...
        {
            Session["ZoomCategoria"] = HttpUtility.HtmlDecode(grdGrid.SelectedRow.Cells[0].Text) + "³";

            if (Session["IncProdutoCategoria"] != null && Convert.ToInt32(Request.QueryString["cad"]) == 1)
            {
                Session["ZoomCategoria"] = HttpUtility.HtmlDecode(grdGrid.SelectedRow.Cells[1].Text) + "³";
                Response.Redirect("~/Pages/Produtos/CadProduto.aspx");
            }
            else
            {
                Response.Redirect("~/Pages/Produtos/CadCategoria.aspx");
            }
            //if (Session["IncProdutoCategoria"] != null)
            //{
            //    List<Produto> listCadProduto = new List<Produto>();
            //    listCadProduto = (List<Produto>)Session["IncProdutoCategoria"];
            //    listCadProduto[0].CodigoCategoria = HttpUtility.HtmlDecode(grdGrid.SelectedRow.Cells[1].Text);
