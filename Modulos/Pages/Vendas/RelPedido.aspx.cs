    ...
    public partial class RelPedido : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (Session["ZoomPedido"] != null)
                {
                    ...
                    doc = docDAL.PesquisarDocumento(Convert.ToDecimal(Session["ZoomPedido"].ToString().Split('³')[0]));
                    if (doc == null)
                        btnVoltar_Click(sender, e);
                    ...
                    if (ListPar.Count > 0)
                    {
                        if (doc.CodigoSituacao == 146 && ListPar[0].TipoListagemPedido == 151 && ListPar[0].ConferePedidos == true)
                        {
                            Session["LISTAGEMPORPEDIDO"] = doc.CodigoDocumento;
        ...
        private void MontaCrystal()
        {
            try { 
                ...
                Doc_PedidoDAL orc = new Doc_PedidoDAL();
                clsValidacao v = new clsValidacao();

                DataTable dt = new DataTable();
                
                dt = orc.RelPedido(Convert.ToDecimal(Session["ZoomPedido"].ToString().Split('³')[0]));
                if(dt.Rows.Count == 0)
                    btnVoltar_Click(null, null);

                RptDoc.Load(Server.MapPath("~/Pages/Vendas/RPT/RelPedido.rpt"));
                ...
                RptDoc.Subreports["RelProdutoDocumento.rpt"].SetDataSource(prodDAL.RelProdutoDocumento(Convert.ToDecimal(Session["ZoomPedido"].ToString().Split('³')[0])));
        ...
        protected void btnEnviarEmail_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Hec_Mail.IN_HTML = 1;
                Hec_Mail.TX_ASSUNTO = Session["NomeEmpresa"].ToString() + " - Pedido Código " + txtCodigoDocumento.Text + @"\ N° "+txtNroDocumento.Text;
                Hec_Mail.TX_CORPO = "";
                ...
                Response.Redirect("~/Pages/HabilUtilitarios/ManGerEmails.aspx?IN=2");
            }
            catch (Exception ex)
            {
                ShowMessage(ex.Message, MessageType.Error);
