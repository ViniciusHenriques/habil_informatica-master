...
/****** Object:  StoredProcedure [dbo].[SP_Cancela_Estoque_Documento]    Script Date: 17/09/2020 16:24:42 ******/
DROP PROCEDURE [dbo].[SP_Cancela_Estoque_Documento]
GO
...
create PROCEDURE [dbo].[SP_Cancela_Estoque_Documento]
...
Set @CdOperacao = Isnull((Select Tp.CD_TP_CTR_PARTIDA From #TMPATDOC as T inner join TIPO_DE_OPERACAO as Tp on Tp.CD_TIPO_OPERACAO = T.CD_TIPO_OPERACAO), 0)
...
/* Vê se Tem Contra Partida*/
if @CdOperacao = 0 
begin
	select 'Tipo de Operação não informado no Documento!' as MsgRetorno, 2 as CodRetorno
	RollBack transaction
	return(2)
end
