...
window.webcam = {
    ...
    swf_url: '../../Scripts/CapturaWebcamASPNET/camutils/webcam.swf', // URI to webcam.swf movie (defaults to cwd) - Leonel F: camutils/ é o diretório onde se encontra o arquivo Flash, em relação à raíz do site
    shutter_url: '../../Scripts/CapturaWebcamASPNET/shutter.mp3', // URI to shutter.mp3 sound //Leonel F.: Este arquivo deve estar no diretório raíz do site. Ao movê-lo, a API não funcionou.
	...
	get_html: function(width, height, server_width, server_height) {
		...
		if (this.ie) {
			...
			html += '<embed id="webcam_movie" src="'+this.swf_url+'" loop="false" menu="false" quality="best" bgcolor="#ffffff" width="'+width+'" height="'+height+'" name="webcam_movie" align="middle" allowScriptAccess="always" allowFullScreen="false" type="application/x-shockwave-flash"  flashvars="'+flashvars+'" />';
		}
	...
	snap: function(url, callback, stealth) {
		...
		return this.get_movie()._snap( this.api_url, this.quality, this.shutter_sound ? 1 : 0, this.stealth ? 1 : 0 );
	},
